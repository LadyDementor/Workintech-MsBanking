...
{
    public static class BranchApi //extension methods yazcağımız için static olmalı
    {
        ...
        private static async Task<Results<Ok<List<BranchResponseDto>>, NotFound>> GetAllBranches(IBranchService service)
        {
            ...
            if (!branches.Any())
                //*Static bir metod old.dan typedResults sınıfını kullanıyoruz.
        ...
        private static async Task<Results<Ok<BranchResponseDto>, BadRequest>> UpdateBranch(IBranchService service, int id, BranchDto branch)
        {
